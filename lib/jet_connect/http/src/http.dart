...
import 'interceptors/jet_modifiers.dart';
...
class JetHttpClient {
  ...
  Future<Response<T>> _performRequest<T>(
    ...
    try {
      ...
      if (!errorSafety) {
        throw JetHttpException(err.toString());
      } else {
        return Response<T>(
          request: newRequest,
          headers: null,
  ...
  Future<Response<T>> send<T>(Request<T> request) async {
    try {
      ...
      if (!errorSafety) {
        throw JetHttpException(e.toString());
      }
