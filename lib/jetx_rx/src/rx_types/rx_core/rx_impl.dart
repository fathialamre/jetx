...
/// global object that registers against `JetX` and `Obx`, and allows the
/// reactivity
/// of those `Widgets` and Rx values.

mixin RxObjectMixin<T> on JetListenable<T> {
  //late T _value;
  ...
  /// Widget (`JetX` or `Obx`) gets unmounted from the Widget tree.
...
abstract class _RxImpl<T> extends JetListenable<T> with RxObjectMixin<T> {
